...
using System;
using Point = Domain.Models.Point;
...
public class Game1 : Game
{
    ...
    private const int TILE_SIZE = 7;

    private Tuple<int, int> GetWindowDimensions()
    {
        return new(WorldMapPerspective.ObjectCoordinates.Length * TILE_SIZE, WorldMapPerspective.ObjectCoordinates[0].Length * TILE_SIZE);
    }

    private Point GetTilePosition(int x, int y)
    {
        return new(x * TILE_SIZE, (WorldMapPerspective.ObjectCoordinates[0].Length - y) * TILE_SIZE);
    }
    ...
    private void ProcessBotState()
    {
        ...
        for (int x = 0; x < WorldMapPerspective.ObjectCoordinates.Length; x++)
        {
            var col = WorldMapPerspective.ObjectCoordinates[x];
    ...
    {
        _spriteBatch.Begin();
        foreach (var tile in opponents)
        {
            DrawTile(new Rectangle(tile.X, tile.Y, 1, 1), Color.Teal);
        }
        _spriteBatch.End();
    }
